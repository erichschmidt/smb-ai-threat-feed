...
- **Today's window:** no new actively-exploited vulnerabilities were added to CISA KEV on 08-24-2026. The eight entries below are the last 7 days. The story is the clocks: Zimbra is due **today**; TrueConf missing-login was due 08-23-2026; the Windows VPN hole was due 08-21-2026.

---

## 1. HIGH — Your AI experiment-tracking tool can still be used to steal cloud keys — check it this morning
**CVE-2026-64849** · CISA confirmed active exploitation **08-19-2026** · federal patch deadline **09-02-2026** · **EPSS 0.0815 (94.5th percentile)**
...
**Why it matters to an SMB running AI tools:** This is the on-lens item: the broken product *is* the AI stack. MLflow often sits next to training data, model registries, and cloud credentials. Copilot/RAG pipelines that pull from the same cloud account inherit the blast radius. No new KEV today does not mean this cooled off — the federal clock still runs to 09-02-2026. If last week's "do we even run this?" question never got an answer, that is this morning's job.
...
## 2. CRITICAL — Your self-hosted email server can be owned by a crafted incoming message — CISA's deadline is today
**CVE-2026-73570** · CISA confirmed active exploitation **08-21-2026** · federal patch deadline **08-24-2026 (today)** · EPSS 0.0104 (61.0th percentile) — **KEV is the signal, not EPSS** · CVSS 8.9/10 = high — remote, no login, but only when SNMP notifications are on
...
## 3. CRITICAL — Your Windows VPN hole is still being attacked — the patch deadline already passed
...
**Why it matters to an SMB running AI tools:** The VPN is the front door for remote staff, backup agents, and the automations that feed Copilot / "chat with your files" tools. If the tunnel service is owned, everything behind it is in play — including the document stores those tools read. This is still the highest-likelihood exploited flaw in this week's KEV window.
...
## 4. HIGH — Your LangChain chat-with-data chain can be tricked into rewriting the database
**CVE-2024-8309** · CIRCL / GHSA (not on CISA KEV) · **EPSS 0.1374 (96.3rd percentile)** — above the 0.1 "patch first" flag · CVSS 9.8/10 = critical — scored as exploitable remotely with no login
...
**What broke:** LangChain's **GraphCypherQAChain (the piece that turns a chat question into a graph-database query)** in the 0.2.5-era packages could be steered by **prompt injection (hiding instructions inside the data the AI reads)** so the generated query became a database write, delete, or data grab. This is not a new CISA listing — scanners and CIRCL are still surfacing leftover 0.2.x installs. If you never used LangChain's graph-chat chain, you're done.

**Why it matters to an SMB running AI tools:** This is a second on-lens item: the broken product *is* the AI framework. "Chat with the knowledge graph" tools often sit on customer, inventory, or ticket data. A leftover unpatched chain means a crafted document or chat turn can become a database change — and anything Copilot-style that reads that graph inherits the poison.
...
**CISA metrics:** this item is from CIRCL + FIRST EPSS, not CISA KEV. CISA has not listed it as actively exploited. Known ransomware-campaign use: not flagged. Treat EPSS 0.1374 (more likely than 96.3% of known flaws) as the prioritization signal, then verify the package version.

---

## 5. WATCH — Ransomware crews are posting retail, services, and Salesforce-connected claims — test a restore
**Ransomware.live sector pulse, 08-23-2026 to 08-24-2026:** 10 recent victim disclosures in this run. **qilin** posted four (manufacturing, professional services, technology). Also in the batch: **krybit** and **coinbasecartel** (retail / e-commerce), **lockbit5** (US professional services), **shinyhunters** (US technology, claiming a large Salesforce-record haul), **emperador** (agriculture / food), **kazu** (US healthcare / clinic software).
**Sources:** [Ransomware.live](https://www.ransomware.live)

**What broke:** Nothing new in your software, necessarily — these are victim disclosures. The pattern is the point: one crew (qilin) is posting a mixed manufacturing/services/tech batch, retail shops are in the dump, and one claim is about CRM records. Practice-management, storefront, and Salesforce-connected tools sit in the same "small office with customer records" world a lot of SMBs actually occupy.

**Why it matters to an SMB running AI tools:** New AI connectors (inbox, drive, CRM, clinic or accounting export) are often the least-watched door. Ransomware crews do not need a clever AI exploit if the storefront or CRM box has never had a restore test and the new automation has no MFA. If your assistant reads those systems, a ransomed shop or CRM export is an AI data-source problem.
...
2. Any AI or automation added this year: turn on **MFA (multi-factor authentication — password plus a phone code or key)** and cut its access to only the folders it needs. If it reads Salesforce or another CRM, confirm that connector is MFA-gated too.
3. If you are in retail, professional services, manufacturing, healthcare, or a small tech shop, tell IT: *"Ransomware.live showed a qilin-heavy mixed dump plus a Salesforce-record claim on 08-23-2026 / 08-24-2026. Walk the backup restore and the new AI connectors today."*
...
- **qilin / krybit / coinbasecartel / lockbit5 / shinyhunters / emperador / kazu:** ransomware groups in today's victim disclosures.
...
- **Remus:** a remote-access trojan family. URLs listed with this name are already attacker infrastructure — block and hunt, don't visit.
...
|---|---|---|---|---|
| krybit | Retail & E-Commerce | DE | 08-24-2026 | [Ransomware.live](https://www.ransomware.live) |
| coinbasecartel | Retail & E-Commerce | DE | 08-23-2026 | [Ransomware.live](https://www.ransomware.live) |
| qilin | Manufacturing | IT | 08-23-2026 | [Ransomware.live](https://www.ransomware.live) |
| lockbit5 | Professional Services | US | 08-23-2026 | [Ransomware.live](https://www.ransomware.live) |
| shinyhunters | Technology | US | 08-23-2026 | [Ransomware.live](https://www.ransomware.live) |
| qilin | Professional Services | US | 08-23-2026 | [Ransomware.live](https://www.ransomware.live) |
| qilin | Technology | CL | 08-23-2026 | [Ransomware.live](https://www.ransomware.live) |
| qilin | Manufacturing | QA | 08-23-2026 | [Ransomware.live](https://www.ransomware.live) |
| emperador | Agriculture and Food Production | ES | 08-23-2026 | [Ransomware.live](https://www.ransomware.live) |
| kazu | Healthcare | US | 08-23-2026 | [Ransomware.live](https://www.ransomware.live) |
...
- **CVE-2025-1889 (EPSS 0.0038, 31.4th pct)** — picklescan missed non-standard pickle extensions in ML model files; upgrade picklescan to **0.0.22 or later** — [GHSA](https://github.com/advisories/GHSA-769v-p64c-89pr) · [Sonatype](https://sites.google.com/sonatype.com/vulnerabilities/cve-2025-1889)

### IOCs (ThreatFox 8 + URLhaus 6, newest — linked)
- **ClearFake domains:** `cup.yourbodybydesign.me`, `www.vendee-bieres-boissons.fr`, `new.icare-group.com` — [Malpedia: ClearFake](https://malpedia.caad.fkie.fraunhofer.de/details/js.clearfake)
- **Shin webshell domain:** `fosame.workers.dev` — [Malpedia: php.shin_webshell](https://malpedia.caad.fkie.fraunhofer.de/details/php.shin_webshell)
- **Remus URLs:** `zakuiru.shop:9048/projects`, `shhsift.click:7647/payments`, `markext.shop:4627/sessions`, `shhsift.click:7647/profiles` — [Malpedia: Remus](https://malpedia.caad.fkie.fraunhofer.de/details/win.remus)
- **URLhaus droppers:** [42.239.150.53:53181/i](https://urlhaus.abuse.ch/url/3907649/) (online) · [42.239.150.53:53181/bin.sh](https://urlhaus.abuse.ch/url/3907648/) (online) · [5.182.210.174/a1dd0f](https://urlhaus.abuse.ch/url/3907619/) (offline) · [5.182.210.174/695814](https://urlhaus.abuse.ch/url/3907620/) (offline) · [5.182.210.174/f81791](https://urlhaus.abuse.ch/url/3907621/) (offline) · [5.182.210.174/e37793](https://urlhaus.abuse.ch/url/3907622/) (offline)

---

*Compiled from public sources · 08-24-2026 · Sources: CISA KEV, CISA Advisories, Ransomware.live, CIRCL, FIRST EPSS, ThreatFox, URLhaus*
